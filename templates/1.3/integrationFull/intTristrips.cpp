...
daeMetaElement * intTristrips::intP::_Meta = NULL;
...
intTristrips::registerElement()
{
	...
	_Meta->setStaticPointerAddress(&intTristrips::_Meta);
...
daeElementRef
intTristrips::intP::create(daeInt bytes)
{
	intTristrips::intPRef ref = new(bytes) intTristrips::intP;
	return ref;
}
...
intTristrips::intP::registerElement()
{
    ...
    _Meta = new daeMetaElement;
    _Meta->setName( "p" );
	_Meta->setStaticPointerAddress(&intTristrips::intP::_Meta);
	_Meta->registerConstructor(intTristrips::intP::create);

	domTristrips::domP::_Meta->setMetaIntegration(_Meta);

	_Meta->setElementSize(sizeof(intTristrips::intP));
	_Meta->validate();
...
void
intTristrips::intP::createFrom(daeElementRef element)
{
	// INSERT CODE TO CREATE YOUR USER DATA HERE
	// _object = new myRuntimeClass;
}
...
void
intTristrips::intP::fromCOLLADAPostProcess()
{
	// INSERT CODE TO POST PROCESS HERE
	// myRuntimeClassType* local = (myRuntimeClassType*)_object;
	// local->renderingContext = MYGLOBAL::getRenderingContext;
...
void
intTristrips::intP::createTo(void* userData)
{
	// INSERT CODE TO CREATE COLLADA DOM OBJECTS HERE
	// _element = new domGeometry;
	// _object = userData;
...
void
intTristrips::intP::toCOLLADAPostProcess()
{
	// INSERT CODE TO POST PROCESS HERE
	// myRuntimeClassType* local = (myRuntimeClassType*)_object;
	// local->renderingContext = MYGLOBAL::getRenderingContext;
