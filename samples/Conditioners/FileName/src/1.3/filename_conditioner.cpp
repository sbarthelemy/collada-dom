...
//		cerr << "filename |"<<thisImage->source.file<<"|\n";
		if(strcmp("file", thisImage->getSource().getProtocol()) != 0)
		{
			thisImage->getSource().print();
			cerr<<"protocol for image element "<<i<<" is "<<thisImage->getSource().getProtocol()<<" no changes made \n";
			continue;
		}
		...
		string finalname = prefix + thisImage->getSource().getFile();
		//cerr << "protocol "<<thisImage->source.protocol<<"\n";
		//cerr << "filename "<<thisImage->source.file<<"\n";
		if(finalname.length() != 0)
		{
			if(verbose)	cerr<<"Image uri "<<thisImage->getSource().getURI()<<" converted to "<<finalname<<"\n"; 
			thisImage->getSource().setURI(finalname.c_str());
		}
		else
		{
