...
このツールは、COLLADA入力データ内で、"file:"プロトコルを持ったURIを使うすべての<image>タグを見つけ出します。最初にURIからベース名（最後の/の後にある文字列）だけを抜き出して、続いてユーザ定義したプレフィックスがこのベース名に追加されます。
...
<image>タグに複雑なURIが含まれている場合、このツールがURIを壊してしまう場合があります。
