...
//printf("<inputs>: ");fflush(stdout);
   ...
   {
    domSampler::domInput*	LdomInput = LdomSampler->getInput_array()[LC];
...
//printf("%s ", LdomInput->semantic);fflush(stdout);
    ...
    if(LdomSource)
    {
     domSource::domTechnique*	LTechnique = _SourceGetCOMMONTechnique(LdomSource);

     if(LTechnique )
     {
      domAccessor*	LdomAccessor = LTechnique->getAccessor_array()[0];
      ...
      if(LdomAccessor)
      {
       ...
       domArray*	LdomArray = (domArray*)LdaeElement;
